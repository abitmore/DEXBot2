...
- `modules/order/logger.js`: Centralized logging with configuration-driven control, change detection, and color-coded output. See [Logging System](docs/LOGGING.md) section.
...
- `modules/order/manager.js`: `OrderManager` class — Core coordinator and state machine. Maintains virtual order grid state (Map + indices), manages fund tracking, and delegates specialized operations to three engines. Implements the **Copy-on-Write (COW) pattern** for safe concurrent rebalancing (see [COPY_ON_WRITE_MASTER_PLAN.md](docs/COPY_ON_WRITE_MASTER_PLAN.md)). Persistence methods are async with AsyncLock protection. Lock system prevents concurrent modifications during async operations.
...
- `modules/order/working_grid.js`: **Copy-on-Write grid wrapper** for safe concurrent rebalancing. Clones master grid for isolated modifications, tracks changes via Modified Set, and builds delta actions. Enables fill events to arrive during rebalancing without corrupting working state. Part of COW pattern (see [COPY_ON_WRITE_MASTER_PLAN.md](docs/COPY_ON_WRITE_MASTER_PLAN.md)).
